...
1. **扫描与标注（已完成）**  
   使用 `python-docx` 遍历 `document.paragraphs`，结合 OOXML 检测表格祖先、绘图、公式等，输出「可改写 / 跳过原因」。

2. **样式与正文范围（已完成 / 可扩展）**  
   通过 `skip_style_substrings` 排除标题、题注等；可选 `body_style_substrings` 白名单（非空时仅允许匹配样式）。

3. **LLM 调用（已完成）**  
   使用 `httpx` 请求 `OPENAI_API_BASE` 下的 `/chat/completions`；批量改写时默认 **最多 5 路并发**（`OPENAI_MAX_CONCURRENT`），并复用同一连接，显著短于纯串行耗时。

4. **写回策略（当前版本）**  
   对 **已通过过滤的纯文字段**，将多个 `run` 折叠到第一个 `run` 并替换文本，**保留段落属性 `w:pPr`**（缩进、段前段后等）。  
   **限制**：段内混排多种字体/加粗等可能被打平；复杂域、公式、图片段落不会被选中改写。

5. **后续可增强（未实现，可开 issue）**  
   - 按书签 `BODY_START`/`BODY_END` 限定正文范围  
   - run 级保留加粗/下标（需解析 `w:rPr`）  
   - Word COM / 商业库提高极端版式保真度  
